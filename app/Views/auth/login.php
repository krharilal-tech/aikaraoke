      ...
      <div class="glass-card p-4 p-md-5">
        ...
        <form method="post" action="<?= e(base_url('login')) ?>">
          ...
          <div class="mb-3">
            ...
            <input type="email" class="form-control form-control-ak" id="email" name="email" required autofocus>
          </div>
          <div class="mb-4">
            <label class="form-label" for="password">Password</label>
            <input type="password" class="form-control form-control-ak" id="password" name="password" required>
          </div>
          <button type="submit" class="gradient-btn btn btn-lg w-100"><i class="bi bi-box-arrow-in-right me-2"></i>Sign In</button>
        </form>

        <p class="text-secondary small text-center mt-4 mb-0">
          New here?
          <a href="<?= e(base_url('register') . ($next !== '' ? '?next=' . urlencode($next) : '')) ?>">Create an account</a>
          &mdash; your first karaoke is free.
        </p>
      </div>
    </div>
  </div>
